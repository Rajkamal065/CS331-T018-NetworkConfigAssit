...
class DiagnosticVerifier:
    ...
    def verify_connectivity(
        ...
        return {
            "status": "PASS" if loss <= max_allowed_loss_percent else "DEGRADED",
            "packet_loss": loss,
            "avg_rtt_ms": avg_rtt
        }

    @staticmethod
    def run_bandwidth_test(server_ip: str, duration: int = 5) -> dict:
        res = NetworkOps.run_cmd(
            f"iperf3 -c {server_ip} -t {duration} -J"
        )
